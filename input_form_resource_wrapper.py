...
def get_resource_public_key(ip_address):
    ssh_public_key = get_command_output(f"{SSH_CMD} {ip_address} \'cat $HOME/.ssh/pw_id_rsa.pub\'")
...
def create_reverse_ssh_tunnel(ip_address, ssh_port):
    ...
    ssh_keys_exists = get_command_output(f"{SSH_CMD} {ip_address} \'ls $HOME/.ssh/pw_id_rsa 2>/dev/null || echo\'")
    print ( "ssh key exists", ssh_keys_exists)
    if not ssh_keys_exists:
        # Create SSH keys
        logger.warning(f'SSH keys not found in {ip_address}:~/.ssh/pw_id_rsa. Creating keys...')
